...
#ifndef INC_API__UNI_ASCII_H
#define INC_API__UNI_ASCII_H
...
#define UNI_ISXDIGIT( X ) \
	( UNI_ISDIGIT( X ) || ( ( X ) >= 'A' && ( X ) <= 'F' ) || \
		( ( X ) >= 'a' && ( X ) <= 'f' ) )
#define UNI_ISPRINT( X ) ( ( X ) >= '0' && ( X ) <= '~' )
#define UNI_ISCNTRL( X ) ( ( X ) < ' ' || ( X ) == 127 )
#define UNI_ISPUNCT( X ) \
	( ( ( X ) >= '!' && ( X ) <= '/' ) || \
		( ( X ) >= ':' && ( X ) <= '@' ) || \
		( ( X ) >= '[' && ( X ) <= '`' ) || \
		( ( X ) >= '{' && ( X ) <= '~' ) )

#define UNI_TOLOWER( X ) \
	( ( ( X ) >= 'A' && ( X ) <= 'Z' ) ? ( X ) + 32 : ( X ) )
#define UNI_TOUPPER( X ) ( ( ( X ) >= 'a' && ( X ) <= 'z' ) ? (X)-32 : ( X ) )

#endif /* INC_API__UNI_ASCII_H */
